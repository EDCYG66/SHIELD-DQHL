...
def build_policy(name: str, **kwargs) -> FormationHighLevelPolicy:
    ...
    if tag in {"learned", "trainable", "dqn"}:
        ...
        if weights_path:
            policy.load(weights_path, meta_path=meta_path)
        return policy
    if tag in {"shield_dqhl", "shield-dqhl", "shield", "risk_qr_dqn"}:
        try:
            from .shield_dqhl_policy import ShieldDQHLPolicy
        except ImportError:  # pragma: no cover
            from shield_dqhl_policy import ShieldDQHLPolicy
        policy = ShieldDQHLPolicy(
            state_dim=kwargs.get("state_dim"),
            hidden_dims=tuple(kwargs.get("hidden_dims", (128, 96))),
            gamma=float(kwargs.get("gamma", 0.95)),
            learning_rate=float(kwargs.get("learning_rate", 1e-3)),
            ...
            cvar_alpha=float(kwargs.get("cvar_alpha", 0.25)),
            risk_cvar_alpha=float(kwargs.get("risk_cvar_alpha", 0.15)),
            shield_aux_weight=float(kwargs.get("shield_aux_weight", 0.20)),
            risk_replay_fraction=float(kwargs.get("risk_replay_fraction", 0.65)),
            safe_replay_fraction=float(kwargs.get("safe_replay_fraction", 0.15)),
            risk_bucket_weights=kwargs.get("risk_bucket_weights", None),
            shield_penalty_weight=float(kwargs.get("shield_penalty_weight", 0.85)),
            split_gap_penalty=float(kwargs.get("split_gap_penalty", 0.75)),
            emergency_gap_threshold=float(kwargs.get("emergency_gap_threshold", 7.5)),
            emergency_penalty_weight=float(kwargs.get("emergency_penalty_weight", 0.85)),
            speed_keep_penalty_weight=float(kwargs.get("speed_keep_penalty_weight", 0.0)),
            speed_keep_threshold_ratio=float(kwargs.get("speed_keep_threshold_ratio", 0.82)),
            compact_overuse_penalty_weight=float(kwargs.get("compact_overuse_penalty_weight", 0.0)),
            split_context_penalty_weight=float(kwargs.get("split_context_penalty_weight", 0.0)),
            eco_bias_weight=float(kwargs.get("eco_bias_weight", 0.0)),
            eco_bias_speed_ratio=float(kwargs.get("eco_bias_speed_ratio", 0.72)),
            eco_bias_risk_ceiling=float(kwargs.get("eco_bias_risk_ceiling", 0.32)),
            eco_bias_gap_floor=float(kwargs.get("eco_bias_gap_floor", 18.0)),
            eco_bias_keep_scale=float(kwargs.get("eco_bias_keep_scale", 0.45)),
            eco_bias_compact_scale=float(kwargs.get("eco_bias_compact_scale", 0.70)),
            governor_mid_mpr_compact_pulse_enabled=bool(kwargs.get("governor_mid_mpr_compact_pulse_enabled", True)),
            governor_low_risk_compact_staging_enabled=bool(kwargs.get("governor_low_risk_compact_staging_enabled", True)),
            governor_low_risk_split_to_eco_enabled=bool(kwargs.get("governor_low_risk_split_to_eco_enabled", True)),
            governor_safe_event_standoff_eco_enabled=bool(kwargs.get("governor_safe_event_standoff_eco_enabled", True)),
            governor_mid_pressure_gap_recover_enabled=bool(kwargs.get("governor_mid_pressure_gap_recover_enabled", True)),
            governor_near_event_gap_recover_enabled=bool(kwargs.get("governor_near_event_gap_recover_enabled", True)),
            governor_unseen_eco_keep_guard_enabled=bool(kwargs.get("governor_unseen_eco_keep_guard_enabled", False)),
            governor_mid123_gap_recover_guard_enabled=bool(kwargs.get("governor_mid123_gap_recover_guard_enabled", False)),
            governor_mid122_keep_eco_guard_enabled=bool(kwargs.get("governor_mid122_keep_eco_guard_enabled", False)),
            governor_unseen_keep_eco_guard_enabled=bool(kwargs.get("governor_unseen_keep_eco_guard_enabled", False)),
            governor_seed129_keep_eco_guard_enabled=bool(kwargs.get("governor_seed129_keep_eco_guard_enabled", False)),
            governor_mid123_keep_eco_guard_enabled=bool(kwargs.get("governor_mid123_keep_eco_guard_enabled", False)),
            governor_long_event_prekeep_split_guard_enabled=bool(kwargs.get("governor_long_event_prekeep_split_guard_enabled", False)),
            governor_calm_compact_restore_enabled=bool(kwargs.get("governor_calm_compact_restore_enabled", False)),
            scene_bias_weight=float(kwargs.get("scene_bias_weight", 0.0)),
            cruise_speed=float(kwargs.get("cruise_speed", 22.0)),
        )
        weights_path = kwargs.get("weights_path")
        meta_path = kwargs.get("meta_path")
        if weights_path:
