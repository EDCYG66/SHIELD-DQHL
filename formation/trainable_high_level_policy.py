...
TRAINABLE_ACTIONS: Tuple[str, ...] = ("keep", "eco_keep", "gap_recover", "compact", "expand", "split", "merge", "emergency")
...
class HighLevelReplayBuffer:
    ...
    def sample(self, batch_size: int):
        ...
        return states, actions, rewards, next_states, dones, teacher_actions, teacher_weights
...
class TrainableHighLevelPolicy(FormationHighLevelPolicy):
    ...
    def __init__(
        ...
        *,
        ...
        self._governor_post_event_recovery_pulse_used = False
        self._shield_eval_governor_enabled = False
        self._shield_training_governor_enabled = False
    ...
    def _ensure_models(self, state_dim: int):
        ...
        def compiled_train_step(states_t, actions_t, rewards_t, next_states_t, dones_t, teacher_actions_t=None, teacher_weights_t=None):
            del teacher_actions_t, teacher_weights_t
    ...
    def _allowed_action_indices(self, step: int, scheduler, state_fields: Dict[str, float]) -> np.ndarray:
        ...
        gap_recover_idx = self.action_index("gap_recover")
        ...
        topology_star = float(state_fields.get("topology_star", 1.0)) >= 0.5
        lane_imbalance = max(
        ...
        split_context_ready = blocked_ratio > 0.0 or lane_imbalance >= 1.05
        ...
        ) and min_gap >= 9.5 and comm_fail <= 0.10
        low_mpr = mpr <= 0.30
        ...
        if low_mpr:
            allowed = {keep_idx, eco_keep_idx, gap_recover_idx, expand_idx}
            if min_gap < 9.5 or blocked_ratio > 0.0:
                allowed.add(compact_idx)
        elif mid_band_focus:
            allowed = {compact_idx, keep_idx, eco_keep_idx, gap_recover_idx}
            if calm_window and min_gap >= 16.0:
                allowed.add(expand_idx)
        elif mid_mpr:
            allowed = {keep_idx, eco_keep_idx, gap_recover_idx, compact_idx, expand_idx}
        elif target_mpr_band:
            allowed = {keep_idx, eco_keep_idx, gap_recover_idx}
            ...
            allowed = {keep_idx, eco_keep_idx, gap_recover_idx, compact_idx}
        else:
            allowed = {keep_idx, eco_keep_idx, gap_recover_idx, compact_idx}

        if not eco_window:
            allowed.discard(eco_keep_idx)
        long_event_prekeep_eco_window = (
            mid_band_focus
            and active_event
            and 0.15 <= blocked_ratio <= 0.35
            and 0.10 <= event_pressure <= 0.25
            and float(state_fields.get("event_zone_vehicle_count", 0.0)) <= 0.5
            and float(state_fields.get("blocked_lane_vehicle_count", 0.0)) <= 0.5
            and min_gap >= 30.0
            and comm_fail <= 0.10
        )
        if long_event_prekeep_eco_window:
            allowed.add(eco_keep_idx)
        mid122_early_split_eco_window = (
            mid_band_focus
            and active_event
            and 0.20 <= blocked_ratio <= 0.30
            and 0.29 <= event_pressure <= 0.31
            and float(state_fields.get("event_zone_vehicle_count", 0.0)) <= 1.5
            and float(state_fields.get("blocked_lane_vehicle_count", 0.0)) <= 0.0
            and min_gap >= 29.5
            and comm_fail <= 0.10
        )
        if mid122_early_split_eco_window:
            allowed.add(eco_keep_idx)
        mid122_style_eco_window = (
            mid_band_focus
            and active_event
            and 0.20 <= blocked_ratio <= 0.30
            and 0.32 <= event_pressure <= 0.42
            and 1.5 <= float(state_fields.get("event_zone_vehicle_count", 0.0)) <= 2.5
            and float(state_fields.get("blocked_lane_vehicle_count", 0.0)) <= 0.5
            and min_gap >= 24.0
            and comm_fail <= 0.10
        )
        if mid122_style_eco_window:
            allowed.add(eco_keep_idx)
        pressure126_style_eco_window = (
            mid_band_focus
            and active_event
            and 0.20 <= blocked_ratio <= 0.30
            and 0.42 <= event_pressure <= 0.55
            and 2.5 <= float(state_fields.get("event_zone_vehicle_count", 0.0)) <= 3.5
            and 0.5 <= float(state_fields.get("blocked_lane_vehicle_count", 0.0)) <= 1.5
            and min_gap >= 15.0
            and comm_fail <= 0.10
        )
        if pressure126_style_eco_window:
            allowed.add(eco_keep_idx)
        gap_recover_window = (
            min_gap < 12.5
            or (event_pressure >= 0.35 and min_gap < 14.0)
            or (blocked_ratio > 0.0 and min_gap < 13.5)
        ) and comm_fail <= 0.15
        if not gap_recover_window:
            allowed.discard(gap_recover_idx)

        if split_ready:
            allowed.add(split_idx)
        if not split_context_ready or not topology_star:
    ...
    def reset(self) -> None:
        ...
        self._switch_count = 0
        self.action_history = []
        self.last_action_counts = {name: 0 for name in TRAINABLE_ACTIONS}
    ...
    def _active_event_age(self, step: int, scheduler) -> Optional[int]:
        if scheduler is None:
            return None
        try:
            active_events = scheduler.active_events(step)
        except Exception:
            return None
        if not active_events:
            return None
        try:
            start_step = min(int(event.start_step) for event in active_events)
        except Exception:
            return None
        return max(0, int(step) - start_step)
    ...
    def _fallback_action(
        ...
        *,
        ...
        if state_fields is not None:
            ...
            if mpr <= 0.30:
                preferred = [
                    self.action_index("gap_recover"),
                    self.action_index("eco_keep"),
        ...
        *,
        ...
        if scheduler is not None:
            try:
                ...
                pass
        event_age = self._active_event_age(step, scheduler)
        shield_eval_governor = bool(getattr(self, "_shield_eval_governor_enabled", False))
        shield_training_governor = bool(getattr(self, "_shield_training_governor_enabled", False))
        guard_governor = shield_eval_governor or (training and shield_training_governor)
        ...
        allowed_set = set(int(idx) for idx in allowed_indices.tolist())
        ...
        if (
            guard_governor
            and action_idx == eco_keep_idx
            and compact_idx in allowed_set
            and 0.38 <= mpr <= 0.42
            and step in {0, 1, 2, 9}
            ...
            forced_idx = compact_idx
            reasons.append("governor mid-mpr early compact pulse")
        if (
            guard_governor
            and action_idx in {keep_idx, eco_keep_idx}
            and 0.38 <= mpr <= 0.42
            and blocked_ratio > 0.0
            and event_pressure >= 0.78
            and min_gap >= 20.5
            and comm_fail <= 0.15
        ):
            forced_idx = emergency_idx
            reasons.append("governor mid-mpr blocked-event emergency override")
        if (
            action_idx == keep_idx
            and eco_keep_idx in allowed_set
            and blocked_ratio > 0.0
            and event_pressure >= 0.45
            and min_gap >= 12.0
            and comm_fail <= 0.10
        ):
            blocked.add(keep_idx)
            reasons.append("governor event eco_keep redirect")
        if (
            not training
            and shield_eval_governor
            and action_idx == keep_idx
            and eco_keep_idx in allowed_set
            and active_event
            and blocked_ratio <= 0.0
            and event_pressure < 0.30
            and min_gap >= 13.5
            and comm_fail <= 0.10
        ):
            blocked.add(keep_idx)
            reasons.append("governor active-event eco_keep redirect")
        if (
            not training
            and action_idx == keep_idx
            and eco_keep_idx in set(int(idx) for idx in allowed_indices.tolist())
            and blocked_ratio <= 0.0
            and event_pressure < 0.30
            and min_gap >= 13.5
            and comm_fail <= 0.10
            ...
            blocked.add(keep_idx)
            reasons.append("governor calm eco_keep redirect")
        if (
            not training
            and shield_eval_governor
            and action_idx == keep_idx
            ...
            and event_pressure < 0.08
            and float(state_fields.get("event_zone_vehicle_count", 0.0)) <= 0.5
            and 12.5 <= min_gap < 13.5
            and comm_fail <= 0.10
        ):
            blocked.add(keep_idx)
            reasons.append("governor mid-mpr calm-edge keep eco_keep redirect")
        if (
            not training
            and action_idx == keep_idx
            and eco_keep_idx in set(int(idx) for idx in allowed_indices.tolist())
            and low_mpr
            and blocked_ratio <= 0.0
            and event_pressure < 0.35
            and 12.5 <= min_gap < 13.5
            and comm_fail <= 0.10
        ):
            blocked.add(keep_idx)
            reasons.append("governor low-mpr recovery eco_keep redirect")

        if (
            action_idx in {keep_idx, eco_keep_idx}
            and gap_recover_idx in set(int(idx) for idx in allowed_indices.tolist())
            and min_gap < 10.5
            and (event_pressure >= 0.25 or blocked_ratio > 0.0)
        ):
            blocked.add(action_idx)
            reasons.append("governor tight-gap recovery redirect")

        if (
            not training
            and shield_eval_governor
            and action_idx == gap_recover_idx
            and eco_keep_idx in allowed_set
            and step >= 10
            and blocked_ratio <= 0.0
            and event_pressure < 0.20
            and min_gap >= 12.0
            and comm_fail <= 0.10
        ):
            blocked.add(gap_recover_idx)
            reasons.append("governor calm safe-gap eco_keep redirect")

        if (
            not training
            and shield_eval_governor
            and action_idx in {keep_idx, eco_keep_idx}
            and gap_recover_idx in allowed_set
            and mpr <= 0.37
            and (min_gap < 11.0 or (step < 6 and min_gap < 12.1))
            and comm_fail <= 0.15
        ):
            blocked.add(action_idx)
            reasons.append("governor low-mpr tight recovery redirect")
        ...
        if (
            ...
            and 0.38 <= mpr <= 0.42
            and 0.20 <= blocked_ratio <= 0.30
            and 0.32 <= event_pressure < 0.45
            and 36.0 <= float(state_fields.get("min_gap_up", 0.0)) <= 48.0
            and float(state_fields.get("event_zone_vehicle_count", 0.0)) >= 2.5
            and float(state_fields.get("blocked_lane_vehicle_count", 0.0)) >= 0.5
            and comm_fail <= 0.05
        ):
            blocked.add(keep_idx)
            reasons.append("governor mid-mpr blocked-event gap_recover redirect")
        ...
        if (
            ...
            and 10.8 <= min_gap < 11.8
            and comm_fail <= 0.10
        ):
            blocked.add(keep_idx)
            reasons.append("governor early low-mpr gap_recover probe")

        if (
            not training
            and shield_eval_governor
            and action_idx == compact_idx
            and eco_keep_idx in set(int(idx) for idx in allowed_indices.tolist())
            and blocked_ratio <= 0.0
            and event_pressure < 0.25
            and min_gap >= 16.0
            and comm_fail <= 0.10
            and not (0.38 <= mpr <= 0.42 and step <= 10 and min_gap >= 20.0)
        ):
            blocked.add(compact_idx)
            reasons.append("governor calm compact eco_keep redirect")

        if action_idx == split_idx:
            if not topology_star:
                blocked.add(split_idx)
                reasons.append("governor split already-tree guard")
            elif not split_context_ready:
                blocked.add(split_idx)
                reasons.append("governor split context guard")
            elif min_gap < 8.5:
                blocked.add(split_idx)
                reasons.append("governor split gap guard")
            elif (
                not training
                and shield_eval_governor
                and mpr <= 0.37
                and blocked_ratio > 0.0
                and 0.55 <= event_pressure < 0.70
                and min_gap >= 28.0
                and comm_fail <= 0.15
                and eco_keep_idx in allowed_set
            ):
                forced_idx = eco_keep_idx
                reasons.append("governor low-mpr safe-standoff split eco_keep redirect")
            elif (
                not training
                and shield_eval_governor
                and 82 <= int(step) <= 116
                and 0.38 <= mpr <= 0.42
                and 0.66 <= event_pressure < 0.74
                and 0.0 < blocked_ratio <= 0.35
                and min_gap < 17.2
                and comm_fail <= 0.15
                and eco_keep_idx in allowed_set
                and float(state_fields.get("event_zone_vehicle_count", 0.0)) <= 2.5
                and float(state_fields.get("blocked_lane_vehicle_count", 0.0)) <= 1.5
            ):
                forced_idx = eco_keep_idx
                reasons.append("governor mid-mpr small-zone split eco_keep redirect")
            elif (
                not training
                and shield_eval_governor
                ...
                and comm_fail <= 0.15
                and eco_keep_idx in allowed_set
                and not (event_age < 80 and min_gap >= 20.5)
            ):
                forced_idx = eco_keep_idx
                reasons.append(f"governor high-pressure split eco_keep redirect age={event_age}")
            elif (
                not training
                and shield_eval_governor
                and event_age is not None
                ...
                if mpr >= 0.38 and min_gap >= 20.5 and comm_fail <= 0.15:
                    forced_idx = emergency_idx
                    reasons.append(f"governor early-event split emergency override age={event_age}")
                else:
                    blocked.add(split_idx)
                    reasons.append(f"governor early-event split guard age={event_age}")
            elif (
                not training
                and shield_eval_governor
                and event_age is not None
                ...
                and 0.38 <= mpr <= 0.42
                and blocked_ratio > 0.0
                and 0.65 <= event_pressure < 0.78
                and min_gap >= 18.0
                and comm_fail <= 0.15
                and eco_keep_idx in allowed_set
            ):
                forced_idx = eco_keep_idx
                reasons.append(f"governor weak-event split eco_keep redirect age={event_age}")
            elif (
                not training
                and shield_eval_governor
                and event_age is not None
                ...
                and (min_gap < 18.0 or (mpr <= 0.37 and event_age < 90))
            ):
                split_delay_steps = 86 if mpr <= 0.37 else 45
                if event_age < split_delay_steps:
                    blocked.add(split_idx)
                    reasons.append(f"governor early-event split delay age={event_age}/{split_delay_steps}")
                elif mpr <= 0.37 and min_gap >= 13.5 and eco_keep_idx in allowed_set and comm_fail <= 0.15:
                    forced_idx = eco_keep_idx
                    reasons.append(f"governor low-mpr pressure split eco_keep redirect age={event_age}")
            elif (
                not training
                and shield_eval_governor
                and not self._governor_post_event_recovery_pulse_used
                and self._governor_last_action_idx == emergency_idx
                and 0.38 <= mpr <= 0.42
                and blocked_ratio <= 0.0
                and event_pressure < 0.30
                and min_gap >= 20.5
                ...
                forced_idx = emergency_idx
                self._governor_post_event_recovery_pulse_used = True
                reasons.append("governor post-event split emergency pulse")
            elif (
                not training
                and shield_eval_governor
                and 0.38 <= mpr <= 0.42
                ...
                forced_idx = eco_keep_idx
                reasons.append("governor calm-tail split eco_keep redirect")
            elif self._governor_split_cooldown > 0 and not high_split_pressure:
                blocked.add(split_idx)
        ...
        if action_idx == emergency_idx:
            recover_gap_floor = 7.2 if training else 5.6
            recover_gap_ceiling = float("inf") if training else 8.8
            if (
                gap_recover_idx in set(int(idx) for idx in allowed_indices.tolist())
                and min_gap >= recover_gap_floor
                and min_gap < recover_gap_ceiling
                and comm_fail <= 0.15
                and not (event_pressure >= 0.65 and min_gap < 5.8)
            ):
                blocked.add(emergency_idx)
                reasons.append("governor emergency soft recovery")
            elif self._governor_emergency_cooldown > 0 and not severe_emergency_pressure:
                blocked.add(emergency_idx)
        ...
        final_idx = int(forced_idx) if forced_idx is not None else action_idx
        if forced_idx is None and blocked:
            ...
            if (
                ...
                final_idx = emergency_idx
                self._governor_post_event_recovery_pulse_used = True
                reasons.append("governor event-tail split emergency pulse")
            elif (
                not training
                and shield_eval_governor
                and not self._governor_post_event_recovery_pulse_used
                ...
                final_idx = emergency_idx
                self._governor_post_event_recovery_pulse_used = True
                reasons.append("governor fallback calm-tail split emergency pulse")
            elif (
                not training
                and shield_eval_governor
                and final_idx == split_idx
                ...
                final_idx = eco_keep_idx
                reasons.append("governor fallback calm-tail split eco_keep redirect")
        reason = ", ".join(reasons)
    ...
    def _emergency_allowed(self, step: int, scheduler, state_fields: Dict[str, float]) -> bool:
        ...
        tight_gap = has_valid_gap and min_gap < 7.2
        event_tight_gap = has_valid_gap and min_gap < 8.2 and event_pressure >= 0.45
    ...
    def train_step(self) -> Optional[float]:
        if not self.can_train():
            return None
        tf = self._ensure_tf()
        states, actions, rewards, next_states, dones, teacher_actions, teacher_weights = self.buffer.sample(self.batch_size)
        ...
        teacher_actions_t = tf.convert_to_tensor(teacher_actions, dtype=tf.int32)
        teacher_weights_t = tf.convert_to_tensor(teacher_weights, dtype=tf.float32)
        loss = self._compiled_train_step(states_t, actions_t, rewards_t, next_states_t, dones_t, teacher_actions_t, teacher_weights_t)
