...
try:
    ...
    from .path_utils import ensure_communication_on_path, ensure_myproject_on_path
...
ensure_myproject_on_path()
ensure_communication_on_path()
from highway_environment import HighwayTopoEnv  # type: ignore  # noqa: E402


HIGH_LEVEL_ACTIONS = ("keep", "eco_keep", "gap_recover", "compact", "expand", "split", "merge", "emergency")
...
class FormationExperimentEnv:
    ...
    def __init__(
        ...
        *,
        ...
        reward_energy_weight: float = 0.0,
        reward_speed_excess_weight: float = 0.0,
        reward_accel_weight: float = 0.0,
        reward_energy_reference_kj: float = 0.20,
        reward_speed_target_ratio: float = 0.82,
        reward_eco_keep_bonus: float = 0.0,
        reward_gap_recover_bonus_weight: float = 0.0,
        reward_eco_context_bonus_weight: float = 0.0,
        reward_eco_postevent_bonus_weight: float = 0.0,
        reward_keep_midpressure_penalty_weight: float = 0.0,
        reward_compact_risk_weight: float = 0.0,
        reward_compact_risk_gap_ref: float = 14.0,
        reward_compact_risk_event_ref: float = 0.30,
        reward_compact_risk_blocked_ref: float = 0.15,
        reward_collision_count_weight: float = 3.0,
        reward_collision_active_weight: float = 1.5,
        reward_emergency_brake_weight: float = 0.30,
        reward_shield_intervention_weight: float = 0.05,
        reward_tight_gap_weight: float = 0.20,
        reward_safe_gap_bonus: float = 0.08,
        **env_kwargs,
    ):
        ...
        self.reward_energy_weight = float(max(0.0, reward_energy_weight))
        self.reward_speed_excess_weight = float(max(0.0, reward_speed_excess_weight))
        self.reward_accel_weight = float(max(0.0, reward_accel_weight))
        self.reward_energy_reference_kj = float(max(1e-6, reward_energy_reference_kj))
        self.reward_speed_target_ratio = float(np.clip(reward_speed_target_ratio, 0.1, 1.5))
        self.reward_eco_keep_bonus = float(max(0.0, reward_eco_keep_bonus))
        self.reward_gap_recover_bonus_weight = float(max(0.0, reward_gap_recover_bonus_weight))
        self.reward_eco_context_bonus_weight = float(max(0.0, reward_eco_context_bonus_weight))
        self.reward_eco_postevent_bonus_weight = float(max(0.0, reward_eco_postevent_bonus_weight))
        self.reward_keep_midpressure_penalty_weight = float(max(0.0, reward_keep_midpressure_penalty_weight))
        self.reward_compact_risk_weight = float(max(0.0, reward_compact_risk_weight))
        self.reward_compact_risk_gap_ref = float(max(1.0, reward_compact_risk_gap_ref))
        self.reward_compact_risk_event_ref = float(max(1e-6, reward_compact_risk_event_ref))
        self.reward_compact_risk_blocked_ref = float(max(1e-6, reward_compact_risk_blocked_ref))
        self.reward_collision_count_weight = float(max(0.0, reward_collision_count_weight))
        self.reward_collision_active_weight = float(max(0.0, reward_collision_active_weight))
        self.reward_emergency_brake_weight = float(max(0.0, reward_emergency_brake_weight))
        self.reward_shield_intervention_weight = float(max(0.0, reward_shield_intervention_weight))
        self.reward_tight_gap_weight = float(max(0.0, reward_tight_gap_weight))
        self.reward_safe_gap_bonus = float(max(0.0, reward_safe_gap_bonus))
    ...
    def current_state(self):
        ...
        # Keep event-zone diagnostics visible to high-level policies without
        # changing the saved model input vector shape.
        state["fields"].update(self._collect_event_stats())
    ...
    def _apply_high_level_action(self, action: str) -> None:
        ...
        if action == "split":
            ...
            action in {"keep", "eco_keep", "gap_recover", "compact"}
            and str(getattr(self.env, "topology_type", "star")).lower() != "star"
            and 0.38 <= float(getattr(self.env, "mpr_cav", 0.0)) <= 0.42
            and self._stale_tree_topology_ready_to_recover()
        ):
            topology_target = "star"
    ...
    def _stale_tree_topology_ready_to_recover(self) -> bool:
        if self.scheduler is None:
            return False
        try:
            if bool(self.scheduler.has_active_event(self.step_count)):
                return False
        except Exception:
            return False
        event_end = int(getattr(self.scheduler, "horizon_end", 0))
        if event_end <= 0 or int(self.step_count) <= event_end:
            return False
        return True
    ...
    def _reward_components(self, fields: Dict[str, float], info: Dict[str, object]) -> Dict[str, float]:
        ...
        safety = (
            -self.reward_collision_count_weight * collision_count
            -self.reward_collision_active_weight * collision_active
            -self.reward_emergency_brake_weight * emergency_brakes
            -self.reward_shield_intervention_weight * interventions
            -self.reward_tight_gap_weight * max(0.0, 8.0 - min_gap) / 8.0
        )
        if min_gap >= 10.0 and interventions <= 0.0 and emergency_brakes <= 0.0:
            safety += self.reward_safe_gap_bonus
        ...
        energy_speed = 0.0
        if self.reward_energy_weight > 0.0 or self.reward_speed_excess_weight > 0.0 or self.reward_accel_weight > 0.0:
            energy_per_vehicle = float(info.get("energy_step_per_vehicle_kj", 0.0))
            energy_norm = float(np.clip(energy_per_vehicle / self.reward_energy_reference_kj, 0.0, 4.0))
            speed_excess = float(np.clip((speed_ratio - self.reward_speed_target_ratio) / max(1e-6, 1.0 - self.reward_speed_target_ratio), 0.0, 3.0))
            max_accel = max(float(getattr(self.controller, "max_accel", 2.5)), abs(float(getattr(self.controller, "max_decel", -4.5))), 1e-6)
            accel_norm = float(np.clip(float(info.get("avg_abs_accel_cmd", 0.0)) / max_accel, 0.0, 2.0))
            safety_relief = 1.0 - 0.55 * float(self._emergency_action_needed(
                fields, info, min_gap=min_gap, event_pressure=event_pressure, blocked_ratio=blocked_ratio
            ))
            gap_relief = 0.45 + 0.55 * float(np.clip((min_gap - 8.5) / 7.5, 0.0, 1.0))
            shaping_scale = float(np.clip(safety_relief * gap_relief, 0.0, 1.0))
            energy_speed = -shaping_scale * (
                self.reward_energy_weight * energy_norm
                + self.reward_speed_excess_weight * speed_excess
                + self.reward_accel_weight * accel_norm
            )

        emergency_needed = self._emergency_action_needed(
            fields, info, min_gap=min_gap, event_pressure=event_pressure, blocked_ratio=blocked_ratio
        )
        action_penalty, action_details = self._action_appropriateness_details(
            fields=fields,
            info=info,
            action=action,
            min_gap=min_gap,
        ...
        return {
            ...
            "energy_speed": float(energy_speed),
            "action_appropriateness": float(action_penalty),
        }

    def _action_appropriateness_details(
        self,
        *,
        ...
        comm_fail: float,
        interventions: float,
        collision_active: float,
        emergency_needed: bool,
    ) -> tuple[float, Dict[str, float]]:
        details: Dict[str, float] = {
            "action_app_emergency_false_penalty": 0.0,
            "action_app_compact_tight_gap_penalty": 0.0,
            "action_app_gap_recover_need": 0.0,
            "action_app_gap_risk_support": 0.0,
            "action_app_gap_recover_base_bonus": 0.0,
            "action_app_gap_recover_context_bonus": 0.0,
            "action_app_gap_recover_idle_penalty": 0.0,
            "action_app_eco_gap_score": 0.0,
            "action_app_eco_event_score": 0.0,
            "action_app_eco_comm_score": 0.0,
            "action_app_eco_keep_bonus": 0.0,
            "action_app_eco_context_score": 0.0,
            "action_app_eco_context_bonus": 0.0,
            "action_app_keep_midpressure_penalty": 0.0,
            "action_app_compact_risk_score": 0.0,
            "action_app_compact_risk_penalty": 0.0,
        }
        action_penalty = 0.0

        if action == "emergency" and not emergency_needed:
            details["action_app_emergency_false_penalty"] = -0.35
            action_penalty += details["action_app_emergency_false_penalty"]
        if action == "compact" and min_gap < 10.0:
            details["action_app_compact_tight_gap_penalty"] = -0.10
            action_penalty += details["action_app_compact_tight_gap_penalty"]
        if action == "gap_recover":
            recover_need = max(
                float(np.clip((12.5 - min_gap) / 5.5, 0.0, 1.0)),
                float(np.clip((event_pressure - 0.30) / 0.50, 0.0, 1.0)) * float(np.clip((14.0 - min_gap) / 6.0, 0.0, 1.0)),
                float(blocked_ratio > 0.0) * float(np.clip((13.5 - min_gap) / 5.5, 0.0, 1.0)),
            )
            details["action_app_gap_recover_need"] = recover_need
            if recover_need > 0.0 and not collision_active:
                details["action_app_gap_recover_base_bonus"] = 0.07 * recover_need
                action_penalty += details["action_app_gap_recover_base_bonus"]
                if self.reward_gap_recover_bonus_weight > 0.0:
                    risk_support = float(
                        np.clip(
                            0.45 * float(np.clip((event_pressure - 0.20) / 0.45, 0.0, 1.0))
                            + 0.35 * float(np.clip(blocked_ratio / 0.20, 0.0, 1.0))
                            + 0.20 * float(np.clip((12.5 - min_gap) / 5.5, 0.0, 1.0)),
                            0.0,
                            1.0,
                        )
                    )
                    details["action_app_gap_risk_support"] = risk_support
                    details["action_app_gap_recover_context_bonus"] = (
                        self.reward_gap_recover_bonus_weight * recover_need * risk_support
                    )
                    action_penalty += details["action_app_gap_recover_context_bonus"]
            elif min_gap >= 14.0 and event_pressure < 0.25 and blocked_ratio <= 0.0:
                details["action_app_gap_recover_idle_penalty"] = -0.08
                action_penalty += details["action_app_gap_recover_idle_penalty"]
        if action == "eco_keep" and self.reward_eco_keep_bonus > 0.0 and not emergency_needed:
            gap_score = float(np.clip((min_gap - 9.5) / 6.5, 0.0, 1.0))
            event_score = float(np.clip((0.45 - event_pressure) / 0.45, 0.0, 1.0))
            comm_score = float(np.clip((0.10 - comm_fail) / 0.10, 0.0, 1.0))
            blocked_score = 1.0 if blocked_ratio <= 0.0 else 0.0
            details["action_app_eco_gap_score"] = gap_score
            details["action_app_eco_event_score"] = event_score
            details["action_app_eco_comm_score"] = comm_score
            details["action_app_eco_keep_bonus"] = self.reward_eco_keep_bonus * gap_score * event_score * comm_score * blocked_score
            action_penalty += details["action_app_eco_keep_bonus"]
        if action == "eco_keep" and self.reward_eco_context_bonus_weight > 0.0 and not emergency_needed:
            safe_gap = float(np.clip((min_gap - 11.0) / 8.0, 0.0, 1.0))
            calm_event = float(np.clip((0.55 - event_pressure) / 0.55, 0.0, 1.0))
            blocked_relief = 1.0 - float(np.clip(blocked_ratio / 0.25, 0.0, 1.0))
            comm_relief = 1.0 - float(np.clip(comm_fail / 0.10, 0.0, 1.0))
            eco_context = float(np.clip(safe_gap * calm_event * blocked_relief * comm_relief, 0.0, 1.0))
            details["action_app_eco_context_score"] = eco_context
            details["action_app_eco_context_bonus"] = self.reward_eco_context_bonus_weight * eco_context
            action_penalty += details["action_app_eco_context_bonus"]
        if action == "eco_keep" and self.reward_eco_postevent_bonus_weight > 0.0 and not emergency_needed:
            active_event = float(fields.get("active_event", 0.0))
            event_zone_vehicle_count = float(fields.get("event_zone_vehicle_count", 0.0))
            blocked_lane_vehicle_count = float(info.get("blocked_lane_vehicle_count", fields.get("blocked_lane_vehicle_count", 0.0)))
            post_event_window = (
                active_event <= 0.0
            ...
            if post_event_window:
                post_event_score = float(np.clip((min_gap - 15.0) / 10.0, 0.0, 1.0))
                details["action_app_eco_context_score"] = max(details["action_app_eco_context_score"], post_event_score)
                details["action_app_eco_context_bonus"] += self.reward_eco_postevent_bonus_weight * post_event_score
                action_penalty += self.reward_eco_postevent_bonus_weight * post_event_score
        if action == "keep" and self.reward_keep_midpressure_penalty_weight > 0.0 and not emergency_needed:
            active_event = float(fields.get("active_event", 0.0))
            event_zone_vehicle_count = float(fields.get("event_zone_vehicle_count", 0.0))
            local_blocked_lane_vehicle_count = float(
                info.get("blocked_lane_vehicle_count", fields.get("blocked_lane_vehicle_count", 0.0))
            )
            midpressure_window = (
                active_event > 0.0
                and
                0.28 <= max(float(fields.get("event_pressure_up", 0.0)), float(fields.get("event_pressure_down", 0.0))) <= 0.33
                and 0.20 <= blocked_ratio <= 0.30
                and local_blocked_lane_vehicle_count <= 0.0
                and event_zone_vehicle_count <= 2.0
                and 28.0 <= min_gap <= 40.0
            )
            if midpressure_window:
                speed_ratio = float(
                    0.5 * (fields.get("mean_speed_up", 0.0) + fields.get("mean_speed_down", 0.0))
                    / max(1.0, self.controller.cruise_speed)
                )
                keep_midpressure_score = float(np.clip((speed_ratio - 0.78) / 0.20, 0.0, 1.0))
                details["action_app_keep_midpressure_penalty"] = -self.reward_keep_midpressure_penalty_weight * keep_midpressure_score
                action_penalty += details["action_app_keep_midpressure_penalty"]
        if action == "compact" and self.reward_compact_risk_weight > 0.0:
            gap_risk = float(np.clip((self.reward_compact_risk_gap_ref - min_gap) / self.reward_compact_risk_gap_ref, 0.0, 1.0))
            event_risk = float(np.clip(event_pressure / self.reward_compact_risk_event_ref, 0.0, 1.0))
            blocked_risk = float(np.clip(blocked_ratio / self.reward_compact_risk_blocked_ref, 0.0, 1.0))
            intervention_risk = float(np.clip(interventions / 2.0, 0.0, 1.0))
            emergency_risk = 1.0 if emergency_needed else 0.0
            compact_risk = float(np.clip(
                0.35 * gap_risk
                + 0.25 * event_risk
                + 0.20 * blocked_risk
                + 0.10 * intervention_risk
                + 0.10 * emergency_risk,
                0.0,
                1.0,
            ))
            details["action_app_compact_risk_score"] = compact_risk
            details["action_app_compact_risk_penalty"] = -self.reward_compact_risk_weight * compact_risk
            action_penalty += details["action_app_compact_risk_penalty"]
        return action_penalty, details
