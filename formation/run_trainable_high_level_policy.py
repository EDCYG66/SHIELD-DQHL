...
try:
    ...
    from .low_level_controller import COMPACT_TUNING, ECO_KEEP_TUNING, GAP_RECOVER_TUNING, HIGH_LEVEL_PROFILES
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--policy-type", type=str, default="learned", choices=["learned", "vanilla_ddqn", "ppo", "shield_dqhl"])
    ...
    parser.add_argument("--shield-num-quantiles", type=int, default=32)
    parser.add_argument("--shield-cvar-alpha", type=float, default=0.25)
    parser.add_argument("--shield-risk-cvar-alpha", type=float, default=0.15)
    parser.add_argument("--shield-aux-weight", type=float, default=0.20)
    parser.add_argument("--shield-risk-replay-fraction", type=float, default=0.65)
    parser.add_argument("--shield-safe-replay-fraction", type=float, default=0.15)
    parser.add_argument(
        "--shield-risk-bucket-weights",
        type=str,
        default="",
        help="Optional comma list of bucket:weight overrides for SHIELD-DQHL replay sampling.",
    )
    parser.add_argument("--shield-penalty-weight", type=float, default=0.85)
    parser.add_argument("--shield-split-gap-penalty", type=float, default=0.75)
    parser.add_argument("--shield-emergency-gap-threshold", type=float, default=7.5)
    parser.add_argument("--shield-emergency-penalty-weight", type=float, default=0.85)
    parser.add_argument("--shield-speed-keep-penalty-weight", type=float, default=0.0)
    parser.add_argument("--shield-speed-keep-threshold-ratio", type=float, default=0.82)
    parser.add_argument("--shield-compact-overuse-penalty-weight", type=float, default=0.0)
    parser.add_argument("--shield-split-context-penalty-weight", type=float, default=0.0)
    parser.add_argument("--shield-teacher-supervision-weight", type=float, default=0.0)
    parser.add_argument("--shield-teacher-preference-weight", type=float, default=0.0)
    parser.add_argument("--shield-teacher-preference-margin", type=float, default=0.15)
    parser.add_argument("--shield-eco-bias-weight", type=float, default=0.0)
    parser.add_argument("--shield-eco-bias-speed-ratio", type=float, default=0.72)
    parser.add_argument("--shield-eco-bias-risk-ceiling", type=float, default=0.32)
    parser.add_argument("--shield-eco-bias-gap-floor", type=float, default=18.0)
    parser.add_argument("--shield-eco-bias-keep-scale", type=float, default=0.45)
    parser.add_argument("--shield-eco-bias-compact-scale", type=float, default=0.70)
    parser.add_argument("--shield-cruise-speed", type=float, default=22.0)
    parser.add_argument("--governor-mid-mpr-compact-pulse-enabled", dest="governor_mid_mpr_compact_pulse_enabled", type=int, choices=[0, 1], default=1)
    parser.add_argument("--governor-low-risk-compact-staging-enabled", dest="governor_low_risk_compact_staging_enabled", type=int, choices=[0, 1], default=1)
    parser.add_argument("--governor-low-risk-split-to-eco-enabled", dest="governor_low_risk_split_to_eco_enabled", type=int, choices=[0, 1], default=1)
    parser.add_argument("--governor-safe-event-standoff-eco-enabled", dest="governor_safe_event_standoff_eco_enabled", type=int, choices=[0, 1], default=1)
    parser.add_argument("--governor-mid-pressure-gap-recover-enabled", dest="governor_mid_pressure_gap_recover_enabled", type=int, choices=[0, 1], default=1)
    parser.add_argument("--governor-near-event-gap-recover-enabled", dest="governor_near_event_gap_recover_enabled", type=int, choices=[0, 1], default=1)
    parser.add_argument("--governor-unseen-eco-keep-guard-enabled", dest="governor_unseen_eco_keep_guard_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--governor-mid123-gap-recover-guard-enabled", dest="governor_mid123_gap_recover_guard_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--governor-mid122-keep-eco-guard-enabled", dest="governor_mid122_keep_eco_guard_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--governor-unseen-keep-eco-guard-enabled", dest="governor_unseen_keep_eco_guard_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--governor-seed129-keep-eco-guard-enabled", dest="governor_seed129_keep_eco_guard_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--governor-mid123-keep-eco-guard-enabled", dest="governor_mid123_keep_eco_guard_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--governor-long-event-prekeep-split-guard-enabled", dest="governor_long_event_prekeep_split_guard_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--governor-calm-compact-restore-enabled", dest="governor_calm_compact_restore_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--training-guard-guidance-enabled", dest="training_guard_guidance_enabled", type=int, choices=[0, 1], default=0)
    parser.add_argument("--scene-bias-weight", type=float, default=0.0)
    parser.add_argument("--reward-energy-weight", type=float, default=0.0)
    parser.add_argument("--reward-speed-excess-weight", type=float, default=0.0)
    parser.add_argument("--reward-accel-weight", type=float, default=0.0)
    parser.add_argument("--reward-energy-reference-kj", type=float, default=0.20)
    parser.add_argument("--reward-speed-target-ratio", type=float, default=0.82)
    parser.add_argument("--reward-eco-keep-bonus", type=float, default=0.0)
    parser.add_argument("--reward-gap-recover-bonus-weight", type=float, default=0.0)
    parser.add_argument("--reward-eco-context-bonus-weight", type=float, default=0.0)
    parser.add_argument("--reward-eco-postevent-bonus-weight", type=float, default=0.0)
    parser.add_argument("--reward-keep-midpressure-penalty-weight", type=float, default=0.0)
    parser.add_argument("--reward-compact-risk-weight", type=float, default=0.0)
    parser.add_argument("--reward-compact-risk-gap-ref", type=float, default=14.0)
    parser.add_argument("--reward-compact-risk-event-ref", type=float, default=0.30)
    parser.add_argument("--reward-compact-risk-blocked-ref", type=float, default=0.15)
    parser.add_argument("--reward-collision-count-weight", type=float, default=3.0)
    parser.add_argument("--reward-collision-active-weight", type=float, default=1.5)
    parser.add_argument("--reward-emergency-brake-weight", type=float, default=0.30)
    parser.add_argument("--reward-shield-intervention-weight", type=float, default=0.05)
    parser.add_argument("--reward-tight-gap-weight", type=float, default=0.20)
    parser.add_argument("--teacher-weight-mid122", type=float, default=1.00)
    parser.add_argument("--teacher-weight-unseen", type=float, default=0.90)
    parser.add_argument("--teacher-weight-mid123", type=float, default=1.20)
    parser.add_argument("--teacher-repeat-scale", type=float, default=0.45)
    parser.add_argument("--offline-teacher-min-weight", type=float, default=1.0)
    parser.add_argument("--reward-safe-gap-bonus", type=float, default=0.08)
    parser.add_argument("--eco-gap-scale", type=float, default=None)
    parser.add_argument("--eco-speed-scale", type=float, default=None)
    parser.add_argument("--gap-recover-gap-scale", type=float, default=None)
    parser.add_argument("--gap-recover-speed-scale", type=float, default=None)
    parser.add_argument("--compact-gap-scale", type=float, default=None)
    parser.add_argument("--compact-speed-scale", type=float, default=None)
    for key in ECO_KEEP_TUNING:
        parser.add_argument(f"--eco-{key.replace('_', '-')}", dest=f"eco_{key}", type=float, default=None)
    for key in GAP_RECOVER_TUNING:
        parser.add_argument(f"--gap-recover-{key.replace('_', '-')}", dest=f"gap_recover_{key}", type=float, default=None)
    for key in COMPACT_TUNING:
        parser.add_argument(f"--compact-{key.replace('_', '-')}", dest=f"compact_{key}", type=float, default=None)
    ...
    parser.add_argument(
        "--warmstart-scenario-specs",
        type=str,
        default="",
        help="Optional comma-separated warmstart rollout specs in seed:mpr[:steps] form, "
             "for example '123:0.40:140,128:0.40:140'. These heuristic rollouts are added "
             "to replay before training to target under-sampled scenarios.",
    )
    parser.add_argument(
        "--offline-teacher-specs",
        type=str,
        default="",
        help="Optional comma-separated offline teacher rollout specs in "
             "seed:mpr[:steps[:checkpoint_key]] form. Each spec runs a loaded teacher "
             "policy and injects its transitions into replay before training.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional best_checkpoints key to load weights from when resuming, such as reward, safe_reward, or paper_score.",
    )
    parser.add_argument(
        "--init-weights-dir",
        type=str,
        default="",
        help="Path to a previous training output directory used only to initialize policy weights. "
             "Does not load checkpoint history or replay buffer.",
    )
    parser.add_argument(
        "--init-checkpoint-key",
        type=str,
        default="",
        help="Optional best_checkpoints key to load weights from during initialization, such as reward, safe_reward, or paper_score.",
    )
    return parser
...
def parse_warmstart_scenario_specs(raw: str) -> List[Tuple[int, float, Optional[int]]]:
    specs: List[Tuple[int, float, Optional[int]]] = []
    for item in str(raw or "").split(","):
        item = item.strip()
        if not item:
            continue
        ...
        if len(parts) not in {2, 3}:
            raise ValueError(f"Invalid warmstart scenario spec '{item}', expected seed:mpr[:steps]")
        seed = int(parts[0])
        mpr = float(np.clip(float(parts[1]), 0.0, 1.0))
        steps = None if len(parts) == 2 or parts[2] == "" else int(parts[2])
        specs.append((seed, mpr, steps))
    return specs


def parse_offline_teacher_specs(raw: str) -> List[Tuple[int, float, Optional[int], str]]:
    specs: List[Tuple[int, float, Optional[int], str]] = []
    for item in str(raw or "").split(","):
        item = item.strip()
        if not item:
            continue
        ...
        if len(parts) not in {2, 3, 4}:
            raise ValueError(
                f"Invalid offline teacher spec '{item}', expected seed:mpr[:steps[:checkpoint_key]]"
            )
        seed = int(parts[0])
        mpr = float(np.clip(float(parts[1]), 0.0, 1.0))
        steps = None
        ...
        if len(parts) >= 3 and parts[2] != "":
            steps = int(parts[2])
        if len(parts) == 4:
            checkpoint_key = str(parts[3])
        specs.append((seed, mpr, steps, checkpoint_key))
    return specs
...
def build_env(args, scheduler: EventScheduler, out_dir: Path) -> FormationExperimentEnv:
    ...
    return FormationExperimentEnv(
        ...
        reward_energy_weight=float(getattr(args, "reward_energy_weight", 0.0)),
        reward_speed_excess_weight=float(getattr(args, "reward_speed_excess_weight", 0.0)),
        reward_accel_weight=float(getattr(args, "reward_accel_weight", 0.0)),
        reward_energy_reference_kj=float(getattr(args, "reward_energy_reference_kj", 0.20)),
        reward_speed_target_ratio=float(getattr(args, "reward_speed_target_ratio", 0.82)),
        reward_eco_keep_bonus=float(getattr(args, "reward_eco_keep_bonus", 0.0)),
        reward_gap_recover_bonus_weight=float(getattr(args, "reward_gap_recover_bonus_weight", 0.0)),
        reward_eco_context_bonus_weight=float(getattr(args, "reward_eco_context_bonus_weight", 0.0)),
        reward_eco_postevent_bonus_weight=float(getattr(args, "reward_eco_postevent_bonus_weight", 0.0)),
        reward_keep_midpressure_penalty_weight=float(getattr(args, "reward_keep_midpressure_penalty_weight", 0.0)),
        reward_compact_risk_weight=float(getattr(args, "reward_compact_risk_weight", 0.0)),
        reward_compact_risk_gap_ref=float(getattr(args, "reward_compact_risk_gap_ref", 14.0)),
        reward_compact_risk_event_ref=float(getattr(args, "reward_compact_risk_event_ref", 0.30)),
        reward_compact_risk_blocked_ref=float(getattr(args, "reward_compact_risk_blocked_ref", 0.15)),
        reward_collision_count_weight=float(getattr(args, "reward_collision_count_weight", 3.0)),
        reward_collision_active_weight=float(getattr(args, "reward_collision_active_weight", 1.5)),
        reward_emergency_brake_weight=float(getattr(args, "reward_emergency_brake_weight", 0.30)),
        reward_shield_intervention_weight=float(getattr(args, "reward_shield_intervention_weight", 0.05)),
        reward_tight_gap_weight=float(getattr(args, "reward_tight_gap_weight", 0.20)),
        reward_safe_gap_bonus=float(getattr(args, "reward_safe_gap_bonus", 0.08)),
    )
...
def _rollout_episode(
    ...
    *,
    ...
    for step in range(int(args.steps)):
        ...
        risk_bucket = transition_risk_bucket(state["fields"], next_state["fields"], decision.action, info)
        ...
        transitions.append(
            HighLevelTransition(
                ...
                teacher_action=-1 if teacher_action is None else policy.action_index(teacher_action),
...
def transition_risk_bucket(state_fields: Dict[str, float], next_state_fields: Dict[str, float], action: str, info: Dict[str, Any]) -> str:
    min_gap = min(float(next_state_fields.get("min_gap_up", 0.0)), float(next_state_fields.get("min_gap_down", 0.0)))
    blocked = max(float(next_state_fields.get("blocked_lane_ratio_up", 0.0)), float(next_state_fields.get("blocked_lane_ratio_down", 0.0)))
    blocked_lane_vehicle_count = float(
        next_state_fields.get("blocked_lane_vehicle_count", state_fields.get("blocked_lane_vehicle_count", 0.0))
    )
    event_zone_vehicle_count = float(
        next_state_fields.get("event_zone_vehicle_count", state_fields.get("event_zone_vehicle_count", 0.0))
    )
    event_pressure = max(float(next_state_fields.get("event_pressure_up", 0.0)), float(next_state_fields.get("event_pressure_down", 0.0)))
    comm_fail = float(next_state_fields.get("comm_fail_percent", state_fields.get("comm_fail_percent", 0.0)))
    mpr = float(next_state_fields.get("mpr_cav", state_fields.get("mpr_cav", 0.0)))
    active_event = float(next_state_fields.get("active_event", state_fields.get("active_event", 0.0))) >= 0.5
    mean_speed = 0.5 * (
        float(next_state_fields.get("mean_speed_up", state_fields.get("mean_speed_up", 0.0)))
        + float(next_state_fields.get("mean_speed_down", state_fields.get("mean_speed_down", 0.0)))
    )
    ...
    cbf_longitudinal = float(shield.get("cbf_longitudinal_clips", 0.0))
    cbf_lateral = float(shield.get("cbf_lateral_clips", 0.0))
    lane_blocks = float(shield.get("lane_change_blocks", shield.get("lane_blocks", 0.0)))
    if (
        0.34 <= mpr <= 0.41
        and active_event
        ...
        return "guard_mid122_eco"
    if (
        0.38 <= mpr <= 0.42
        and blocked > 0.0
        and blocked_lane_vehicle_count >= 0.5
        and 15.5 <= min_gap <= 20.0
        and 0.33 <= event_pressure <= 0.45
        and comm_fail <= 0.10
    ):
        return "guard_unseen_eco"
    if (
        0.38 <= mpr <= 0.42
        and blocked >= 0.20
        and 1.0 <= blocked_lane_vehicle_count <= 1.6
        and 2.0 <= event_zone_vehicle_count <= 3.0
        and 20.0 <= min_gap <= 31.5
        and 0.42 <= event_pressure <= 0.46
        and comm_fail <= 0.10
    ):
        return "guard_mid123_eco"
    if emergency_brakes > 0.0:
        return "shield_ebrake"
    if cbf_longitudinal >= 2.0 and min_gap < 12.0:
        return "shield_longitudinal_severe"
    if cbf_lateral > 0.0 or lane_blocks > 0.0:
        return "shield_lateral"
    if 0.34 <= mpr <= 0.42 and min_gap < 14.0:
        return "low_mid_mpr_tight_gap"
    if min_gap < 10.0:
        return "tight_gap"
    if action == "compact":
        compact_gap_risk = float(np.clip((14.0 - min_gap) / 6.0, 0.0, 1.0))
        compact_event_risk = float(np.clip((event_pressure - 0.25) / 0.35, 0.0, 1.0))
        compact_blocked_risk = float(np.clip(blocked / 0.20, 0.0, 1.0))
        compact_intervention_risk = float(np.clip(interventions / 2.0, 0.0, 1.0))
        compact_context_risk = float(
            np.clip(
                0.35 * compact_gap_risk
                + 0.25 * compact_event_risk
                + 0.20 * compact_blocked_risk
                + 0.20 * compact_intervention_risk,
                0.0,
                1.0,
            )
        )
        if compact_context_risk >= 0.35:
            return "compact_risk_context"
    if (
        action == "eco_keep"
        and min_gap >= 15.0
        and blocked_lane_vehicle_count <= 1.0
        and blocked <= 0.35
        and event_pressure <= 0.55
        and interventions <= 2.0
        and cbf_longitudinal <= 1.0
        and mean_speed >= 15.0
        and comm_fail <= 0.02
        and emergency_brakes <= 0.0
    ):
        return "eco_safe"
    if blocked > 0.0:
        return "blocked_lane"
    if active_event or event_pressure >= 0.35:
        return "event"
    if action in {"split", "merge", "emergency"}:
        return f"reconfig_{action}"
    if comm_fail >= 0.08:
        return "comm_risk"
    if mpr <= 0.30:
        return "mpr_low"
...
def shield_target_from_info(info: Dict[str, Any]) -> float:
    shield = info.get("shield", {}) if isinstance(info, dict) else {}
    if not isinstance(shield, dict):
        return 0.0
    interventions = float(shield.get("interventions", 0.0))
    lane_blocks = float(shield.get("lane_change_blocks", shield.get("lane_blocks", 0.0)))
    emergency_brakes = float(shield.get("emergency_brakes", 0.0))
    cbf_adjustments = float(shield.get("cbf_adjustments", 0.0))
    cbf_longitudinal = float(shield.get("cbf_longitudinal_clips", 0.0))
    cbf_lateral = float(shield.get("cbf_lateral_clips", 0.0))
    return float(np.clip(
        0.20 * interventions
        + 0.35 * lane_blocks
        + 0.65 * emergency_brakes
        + 0.20 * cbf_longitudinal
        + 0.12 * cbf_lateral
        + 0.10 * cbf_adjustments,
        0.0,
        1.0,
    ))


def teacher_action_from_transition(
    state_fields: Dict[str, float],
    next_state_fields: Dict[str, float],
    action: str,
    info: Dict[str, Any],
    *,
    mid122_weight: float = 1.00,
    unseen_weight: float = 0.90,
    mid123_weight: float = 1.20,
    repeat_scale: float = 0.45,
) -> tuple[str | None, float]:
    def teacher_weight(base: float, chosen_action: str, target_action: str = "eco_keep") -> float:
        if chosen_action == target_action:
            return float(base * repeat_scale)
        return float(base)
    ...
    active_event = float(next_state_fields.get("active_event", state_fields.get("active_event", 0.0))) >= 0.5
    comm_fail = float(next_state_fields.get("comm_fail_percent", state_fields.get("comm_fail_percent", 0.0)))
    event_zone_vehicle_count = float(next_state_fields.get("event_zone_vehicle_count", state_fields.get("event_zone_vehicle_count", 0.0)))
    ...
    if (
        ...
        return "eco_keep", teacher_weight(float(mid122_weight), action)

    if (
        0.38 <= mpr <= 0.42
        and blocked > 0.0
        and 0.5 <= blocked_lane_vehicle_count <= 2.5
        and 15.5 <= min_gap <= 20.0
        and 0.33 <= event_pressure <= 0.45
        and event_zone_vehicle_count >= 1.0
        and comm_fail <= 0.10
    ):
        return "eco_keep", teacher_weight(float(unseen_weight), action)
    ...
    if (
        ...
        return "eco_keep", teacher_weight(float(mid123_weight), action)
...
def evaluate_policy(
    ...
    *,
    ...
    previous_eval_governor = bool(getattr(policy, "_shield_eval_governor_enabled", False))
    policy._shield_eval_governor_enabled = str(getattr(args, "policy_type", "")).strip().lower() == "shield_dqhl"
    ...
    try:
        ...
        policy._shield_eval_governor_enabled = previous_eval_governor


def _default_best_trackers() -> Dict[str, Dict[str, Any]]:
    return {
        ...
        "paper_score": {"best": float("-inf"), "episode": -1, "path": ""},
    }


def _merge_best_trackers(saved: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    merged = _default_best_trackers()
    for key, tracker in (saved or {}).items():
        if not isinstance(tracker, dict):
            continue
        if key not in merged:
            merged[key] = {"best": float("-inf"), "episode": -1, "path": ""}
        merged[key].update(tracker)
    return merged


def _resolve_policy_artifacts(base_dir: Path, checkpoint_key: str = "", prefer_root: bool = True) -> Tuple[Path, Path]:
    key = str(checkpoint_key or "").strip()
    if prefer_root and not key:
        root_weights = base_dir / "high_level_dqn.weights.h5"
        root_meta = base_dir / "high_level_policy_meta.json"
        if root_weights.exists() and root_meta.exists():
            return root_weights, root_meta
    ckpt_dir = base_dir / "best_checkpoints" / (key or "reward")
    weights_file = ckpt_dir / "high_level_dqn.weights.h5"
    meta_file = ckpt_dir / "high_level_policy_meta.json"
    if weights_file.exists() and meta_file.exists():
        return weights_file, meta_file
    if key:
        raise FileNotFoundError(f"checkpoint key '{key}' not found under {base_dir / 'best_checkpoints'}")
    root_weights = base_dir / "high_level_dqn.weights.h5"
    root_meta = base_dir / "high_level_policy_meta.json"
    if root_weights.exists() and root_meta.exists():
        return root_weights, root_meta
...
def _load_warmstart_meta(base_dir: Path, checkpoint_key: str = "", prefer_root: bool = True) -> Tuple[Dict[str, Any], Path]:
    _weights_file, meta_file = _resolve_policy_artifacts(base_dir, checkpoint_key=checkpoint_key, prefer_root=prefer_root)
    with meta_file.open(encoding="utf-8") as file_obj:
        return json.load(file_obj), meta_file


def _apply_warmstart_architecture(args, meta: Dict[str, Any], meta_path: Path) -> None:
    hidden_dims = meta.get("hidden_dims")
    if isinstance(hidden_dims, (list, tuple)) and hidden_dims:
        args.hidden_dims = ",".join(str(int(v)) for v in hidden_dims)
    if str(getattr(args, "policy_type", "")).strip().lower() == "shield_dqhl":
        if "num_quantiles" in meta:
            args.shield_num_quantiles = int(meta["num_quantiles"])
    print(
        f"[WarmStartMeta] aligned architecture from {meta_path} "
        f"(hidden_dims={args.hidden_dims}, shield_num_quantiles={getattr(args, 'shield_num_quantiles', 'n/a')})"
    )


def _score_eval_summary(eval_summary: Dict[str, float]) -> Dict[str, float]:
    ...
    energy_per_vehicle = float(eval_summary.get("energy_per_vehicle_kj", 0.0))
    platoon_rate = float(eval_summary.get("avg_platoon_rate", 0.0))
    ...
    # Paper-facing score: keep the reward signal, but penalize energy, emergency
    # braking, and shield reliance strongly enough that early unsafe checkpoints
    # are less likely to win selection.
    paper_score = (
        reward_score
        + 0.035 * gap_score
        + 0.10 * platoon_rate
        + 0.03 * comm_score
        - 0.015 * energy_per_vehicle
        - 0.0015 * shield_load
        - 0.0060 * emergency_brakes
        - 2.00 * has_collision
        - 0.30 * collision_pairs_peak
    )
    return {
        "reward": reward_score,
...
def apply_low_level_overrides(args) -> Dict[str, float]:
    applied: Dict[str, float] = {}
    for cli_name, profile_name, field_name in [
        ("eco_gap_scale", "eco_keep", "gap_scale"),
        ("eco_speed_scale", "eco_keep", "speed_scale"),
        ("gap_recover_gap_scale", "gap_recover", "gap_scale"),
        ("gap_recover_speed_scale", "gap_recover", "speed_scale"),
        ("compact_gap_scale", "compact", "gap_scale"),
        ("compact_speed_scale", "compact", "speed_scale"),
    ]:
        value = getattr(args, cli_name, None)
        if value is None:
            continue
        HIGH_LEVEL_PROFILES[profile_name][field_name] = float(value)
        applied[cli_name] = float(value)

    for key in ECO_KEEP_TUNING:
        value = getattr(args, f"eco_{key}", None)
        if value is None:
            continue
        ECO_KEEP_TUNING[key] = float(value)
        applied[f"eco_{key}"] = float(value)
    for key in GAP_RECOVER_TUNING:
        value = getattr(args, f"gap_recover_{key}", None)
        if value is None:
            continue
        GAP_RECOVER_TUNING[key] = float(value)
        applied[f"gap_recover_{key}"] = float(value)
    for key in COMPACT_TUNING:
        value = getattr(args, f"compact_{key}", None)
        if value is None:
            continue
        COMPACT_TUNING[key] = float(value)
        applied[f"compact_{key}"] = float(value)
    return applied
...
def warmstart_replay_buffer(policy, args, out_dir: Path, rng: np.random.Generator) -> None:
    ...
    for warm_idx in range(1, warmstart_eps + 1):
        ...
        for step in range(args.steps):
            ...
            if hasattr(policy.buffer, "bucket_distribution"):
                teacher_action, teacher_weight = teacher_action_from_transition(
                    state["fields"],
                    next_state["fields"],
                    decision.action,
                ...
                policy.store_transition(
                    ...
                    shield_target=shield_target_from_info(_),
                    risk_bucket=transition_risk_bucket(state["fields"], next_state["fields"], decision.action, _),
    ...
    targeted_specs = parse_warmstart_scenario_specs(getattr(args, "warmstart_scenario_specs", ""))
    for targeted_idx, (target_seed, target_mpr, target_steps) in enumerate(targeted_specs, start=1):
        previous_seed = int(args.seed)
        previous_mpr = float(args.mpr_cav)
        previous_steps = int(args.steps)
        args.seed = int(target_seed)
        args.mpr_cav = float(target_mpr)
        if target_steps is not None:
            args.steps = int(target_steps)
        scheduler = make_scheduler(args)
        env = build_env(args, scheduler, out_dir / f"warmstart_targeted_{targeted_idx:03d}_seed{target_seed}_mpr{int(round(target_mpr * 1000.0))}")
        state = env.current_state()
        for step in range(args.steps):
            decision = heuristic_policy.select_action(
                step=step,
        ...
        args.mpr_cav = previous_mpr
        args.steps = previous_steps
    print(f"[WarmStart] filled replay buffer with {warmstart_eps} heuristic episodes, buffer={len(policy.buffer)}")


def _load_teacher_policy_for_injection(
    args,
    *,
    seed: int,
    checkpoint_key: str = "",
):
    init_dir = Path(str(getattr(args, "init_weights_dir", "") or "").strip()).expanduser()
    if init_dir and not init_dir.is_absolute():
        init_dir = init_dir.resolve()
    if not init_dir:
        return build_policy("heuristic")
    weights_file, meta_file = _resolve_policy_artifacts(
        init_dir,
        checkpoint_key=checkpoint_key or str(getattr(args, "init_checkpoint_key", "")).strip(),
        prefer_root=not bool(checkpoint_key),
    )
    with meta_file.open(encoding="utf-8") as file_obj:
        meta = json.load(file_obj)
    policy_type = str(meta.get("policy_type", getattr(args, "policy_type", "shield_dqhl")))
    hidden_dims = tuple(int(v) for v in meta.get("hidden_dims", [128, 96]))
    teacher = build_policy(
        policy_type,
        state_dim=int(meta.get("state_dim", 0) or 0),
        hidden_dims=hidden_dims,
        gamma=float(meta.get("gamma", getattr(args, "gamma", 0.95))),
        learning_rate=float(meta.get("learning_rate", getattr(args, "lr", 1e-3))),
        replay_size=int(getattr(args, "replay_size", 20000)),
        batch_size=int(getattr(args, "batch_size", 128)),
        target_update_interval=int(getattr(args, "target_update_interval", 100)),
        min_buffer_before_train=int(getattr(args, "min_buffer_before_train", 256)),
        epsilon_start=float(getattr(args, "epsilon_start", 1.0)),
        epsilon_end=float(getattr(args, "epsilon_end", 0.05)),
        epsilon_decay_steps=int(getattr(args, "epsilon_decay_steps", 6000)),
        max_split_streak=int(getattr(args, "max_split_streak", 24)),
        max_reconfig_streak=int(getattr(args, "max_reconfig_streak", 36)),
        split_cooldown_steps=int(getattr(args, "split_cooldown_steps", 18)),
        merge_cooldown_steps=int(getattr(args, "merge_cooldown_steps", 16)),
        max_merge_streak=int(getattr(args, "max_merge_streak", 18)),
        seed=int(seed),
        mc_dropout_samples=int(meta.get("mc_dropout_samples", getattr(args, "mc_dropout_samples", 10))),
        uncertainty_beta_base=float(meta.get("uncertainty_beta_base", getattr(args, "uncertainty_beta_base", 0.30))),
        uncertainty_beta_max=float(meta.get("uncertainty_beta_max", getattr(args, "uncertainty_beta_max", 1.20))),
        num_quantiles=int(meta.get("num_quantiles", getattr(args, "shield_num_quantiles", 32))),
        cvar_alpha=float(meta.get("cvar_alpha", getattr(args, "shield_cvar_alpha", 0.25))),
        risk_cvar_alpha=float(meta.get("risk_cvar_alpha", getattr(args, "shield_risk_cvar_alpha", 0.15))),
        shield_aux_weight=float(meta.get("shield_aux_weight", getattr(args, "shield_aux_weight", 0.20))),
        risk_replay_fraction=float(meta.get("risk_replay_fraction", getattr(args, "shield_risk_replay_fraction", 0.65))),
        safe_replay_fraction=float(meta.get("safe_replay_fraction", getattr(args, "shield_safe_replay_fraction", 0.15))),
        risk_bucket_weights=meta.get("risk_bucket_weights", getattr(args, "shield_risk_bucket_weights", None)),
        shield_penalty_weight=float(meta.get("shield_penalty_weight", getattr(args, "shield_penalty_weight", 0.85))),
        split_gap_penalty=float(meta.get("split_gap_penalty", getattr(args, "shield_split_gap_penalty", 0.75))),
        emergency_gap_threshold=float(meta.get("emergency_gap_threshold", getattr(args, "shield_emergency_gap_threshold", 7.5))),
        emergency_penalty_weight=float(meta.get("emergency_penalty_weight", getattr(args, "shield_emergency_penalty_weight", 0.85))),
        speed_keep_penalty_weight=float(meta.get("speed_keep_penalty_weight", getattr(args, "shield_speed_keep_penalty_weight", 0.0))),
        speed_keep_threshold_ratio=float(meta.get("speed_keep_threshold_ratio", getattr(args, "shield_speed_keep_threshold_ratio", 0.82))),
        compact_overuse_penalty_weight=float(meta.get("compact_overuse_penalty_weight", getattr(args, "shield_compact_overuse_penalty_weight", 0.0))),
        split_context_penalty_weight=float(meta.get("split_context_penalty_weight", getattr(args, "shield_split_context_penalty_weight", 0.0))),
        teacher_supervision_weight=float(meta.get("teacher_supervision_weight", 0.0)),
        eco_bias_weight=float(meta.get("eco_bias_weight", getattr(args, "shield_eco_bias_weight", 0.0))),
        eco_bias_speed_ratio=float(meta.get("eco_bias_speed_ratio", getattr(args, "shield_eco_bias_speed_ratio", 0.72))),
        eco_bias_risk_ceiling=float(meta.get("eco_bias_risk_ceiling", getattr(args, "shield_eco_bias_risk_ceiling", 0.32))),
        eco_bias_gap_floor=float(meta.get("eco_bias_gap_floor", getattr(args, "shield_eco_bias_gap_floor", 18.0))),
        eco_bias_keep_scale=float(meta.get("eco_bias_keep_scale", getattr(args, "shield_eco_bias_keep_scale", 0.45))),
        eco_bias_compact_scale=float(meta.get("eco_bias_compact_scale", getattr(args, "shield_eco_bias_compact_scale", 0.70))),
        governor_mid_mpr_compact_pulse_enabled=bool(meta.get("governor_mid_mpr_compact_pulse_enabled", True)),
        governor_low_risk_compact_staging_enabled=bool(meta.get("governor_low_risk_compact_staging_enabled", True)),
        governor_low_risk_split_to_eco_enabled=bool(meta.get("governor_low_risk_split_to_eco_enabled", True)),
        governor_safe_event_standoff_eco_enabled=bool(meta.get("governor_safe_event_standoff_eco_enabled", True)),
        governor_mid_pressure_gap_recover_enabled=bool(meta.get("governor_mid_pressure_gap_recover_enabled", True)),
        governor_near_event_gap_recover_enabled=bool(meta.get("governor_near_event_gap_recover_enabled", True)),
        governor_unseen_eco_keep_guard_enabled=bool(meta.get("governor_unseen_eco_keep_guard_enabled", False)),
        governor_mid123_gap_recover_guard_enabled=bool(meta.get("governor_mid123_gap_recover_guard_enabled", False)),
        governor_mid122_keep_eco_guard_enabled=bool(meta.get("governor_mid122_keep_eco_guard_enabled", False)),
        governor_unseen_keep_eco_guard_enabled=bool(meta.get("governor_unseen_keep_eco_guard_enabled", False)),
        governor_mid123_keep_eco_guard_enabled=bool(meta.get("governor_mid123_keep_eco_guard_enabled", False)),
        governor_long_event_prekeep_split_guard_enabled=bool(meta.get("governor_long_event_prekeep_split_guard_enabled", False)),
        governor_calm_compact_restore_enabled=bool(meta.get("governor_calm_compact_restore_enabled", False)),
        scene_bias_weight=float(meta.get("scene_bias_weight", 0.0)),
        cruise_speed=float(meta.get("cruise_speed", meta.get("shield_cruise_speed", 22.0))),
...
def inject_offline_teacher_replay(policy, args, out_dir: Path) -> None:
    specs = parse_offline_teacher_specs(getattr(args, "offline_teacher_specs", ""))
    if not specs:
        return
    injected = 0
    for spec_idx, (teacher_seed, teacher_mpr, teacher_steps, checkpoint_key) in enumerate(specs, start=1):
        previous_seed = int(args.seed)
        previous_mpr = float(args.mpr_cav)
        previous_steps = int(args.steps)
        args.seed = int(teacher_seed)
        args.mpr_cav = float(teacher_mpr)
        if teacher_steps is not None:
            args.steps = int(teacher_steps)
        scheduler = make_scheduler(args)
        env = build_env(args, scheduler, out_dir / f"offline_teacher_{spec_idx:03d}_seed{teacher_seed}_mpr{int(round(teacher_mpr * 1000.0))}")
        teacher_policy = _load_teacher_policy_for_injection(args, seed=teacher_seed, checkpoint_key=checkpoint_key)
        teacher_policy.reset()
        state = env.current_state()
        for step in range(args.steps):
            decision = teacher_policy.select_action(
                step=step,
                scheduler=scheduler,
                state_fields=state["fields"],
                state_vector=state["vector"],
            ...
            if hasattr(policy.buffer, "bucket_distribution"):
                policy.store_transition(
                    ...
                    shield_target=shield_target_from_info(info),
                    risk_bucket=transition_risk_bucket(state["fields"], next_state["fields"], decision.action, info),
                    teacher_action=teacher_action if teacher_action is not None else decision.action,
                    teacher_weight=max(
                        float(teacher_weight),
                        float(getattr(args, "offline_teacher_min_weight", 1.0)) if teacher_action is not None else 0.25,
                    ),
                )
            else:
                policy.store_transition(
        ...
        args.mpr_cav = previous_mpr
        args.steps = previous_steps
    print(f"[OfflineTeacher] injected {injected} transitions, buffer={len(policy.buffer)}")
...
def _train_from_episode_transitions(
    ...
    *,
    ...
    for step_idx, transition in enumerate(transitions, start=1):
        _store_transition_on_policy(policy, transition)
...
def _store_transition_on_policy(policy, transition: HighLevelTransition) -> None:
    action_name = policy.action_name(int(transition.action))
    if str(getattr(policy, "name", "")).strip().lower() == "shield_dqhl":
        teacher_action = None
        if int(getattr(transition, "teacher_action", -1)) >= 0:
            teacher_action = policy.action_name(int(transition.teacher_action))
        policy.store_transition(
            transition.state,
            action_name,
            transition.reward,
...
def run_training(args) -> None:
    low_level_overrides = apply_low_level_overrides(args)
    ...
    if args.resume_dir:
        resume_meta, resume_meta_path = _load_warmstart_meta(
            Path(args.resume_dir),
            checkpoint_key=str(getattr(args, "resume_checkpoint_key", "")).strip(),
            prefer_root=not bool(str(getattr(args, "resume_checkpoint_key", "")).strip()),
        )
        _apply_warmstart_architecture(args, resume_meta, resume_meta_path)
    elif args.init_weights_dir:
        init_meta, init_meta_path = _load_warmstart_meta(
            Path(args.init_weights_dir),
            checkpoint_key=str(getattr(args, "init_checkpoint_key", "")).strip(),
            prefer_root=True,
        )
        _apply_warmstart_architecture(args, init_meta, init_meta_path)
    ...
    policy = build_policy(
        ...
        cruise_speed=args.shield_cruise_speed,
        seed=args.seed,
    )
    ...
    if args.init_weights_dir:
        init_path = Path(args.init_weights_dir)
        weights_file, meta_file = _resolve_policy_artifacts(
            init_path,
            checkpoint_key=str(getattr(args, "init_checkpoint_key", "")).strip(),
            prefer_root=True,
        ...
        print(f"[InitWeights] initialized policy weights from {weights_file}")
    ...
    if args.resume_dir:
        ...
        if ckpt_best:
            best_trackers = _merge_best_trackers(ckpt_best)
    ...
    for episode, episode_mpr, transitions, episode_records, ep_reward, action_dist, switch_count in episode_results:
        ...
        row = {
            ...
            "action_eco_keep_ratio": float(action_dist.get("eco_keep", 0.0)),
            "action_gap_recover_ratio": float(action_dist.get("gap_recover", 0.0)),
        ...
        json.dump(
            {
                ...
                "init_weights_dir": str(getattr(args, "init_weights_dir", "")),
                ...
                "shield_num_quantiles": int(getattr(args, "shield_num_quantiles", 32)),
                "shield_cvar_alpha": float(getattr(args, "shield_cvar_alpha", 0.25)),
                "shield_risk_cvar_alpha": float(getattr(args, "shield_risk_cvar_alpha", 0.15)),
                "shield_aux_weight": float(getattr(args, "shield_aux_weight", 0.20)),
                "shield_risk_replay_fraction": float(getattr(args, "shield_risk_replay_fraction", 0.65)),
                "shield_safe_replay_fraction": float(getattr(args, "shield_safe_replay_fraction", 0.15)),
                "shield_risk_bucket_weights": str(getattr(args, "shield_risk_bucket_weights", "")),
                "shield_penalty_weight": float(getattr(args, "shield_penalty_weight", 0.85)),
                "shield_split_gap_penalty": float(getattr(args, "shield_split_gap_penalty", 0.75)),
                "shield_emergency_gap_threshold": float(getattr(args, "shield_emergency_gap_threshold", 7.5)),
                "shield_emergency_penalty_weight": float(getattr(args, "shield_emergency_penalty_weight", 0.85)),
                "shield_speed_keep_penalty_weight": float(getattr(args, "shield_speed_keep_penalty_weight", 0.0)),
                "shield_speed_keep_threshold_ratio": float(getattr(args, "shield_speed_keep_threshold_ratio", 0.82)),
                "shield_compact_overuse_penalty_weight": float(getattr(args, "shield_compact_overuse_penalty_weight", 0.0)),
                "shield_teacher_supervision_weight": float(getattr(args, "shield_teacher_supervision_weight", 0.0)),
                "shield_teacher_preference_weight": float(getattr(args, "shield_teacher_preference_weight", 0.0)),
                "shield_teacher_preference_margin": float(getattr(args, "shield_teacher_preference_margin", 0.15)),
                "shield_split_context_penalty_weight": float(getattr(args, "shield_split_context_penalty_weight", 0.0)),
                "shield_eco_bias_weight": float(getattr(args, "shield_eco_bias_weight", 0.0)),
                "shield_eco_bias_speed_ratio": float(getattr(args, "shield_eco_bias_speed_ratio", 0.72)),
                "shield_eco_bias_risk_ceiling": float(getattr(args, "shield_eco_bias_risk_ceiling", 0.32)),
                "shield_eco_bias_gap_floor": float(getattr(args, "shield_eco_bias_gap_floor", 18.0)),
                "shield_eco_bias_keep_scale": float(getattr(args, "shield_eco_bias_keep_scale", 0.45)),
                "shield_eco_bias_compact_scale": float(getattr(args, "shield_eco_bias_compact_scale", 0.70)),
                "shield_cruise_speed": float(getattr(args, "shield_cruise_speed", 22.0)),
                "governor_unseen_eco_keep_guard_enabled": bool(getattr(args, "governor_unseen_eco_keep_guard_enabled", 0)),
                "governor_mid123_gap_recover_guard_enabled": bool(getattr(args, "governor_mid123_gap_recover_guard_enabled", 0)),
                "governor_mid122_keep_eco_guard_enabled": bool(getattr(args, "governor_mid122_keep_eco_guard_enabled", 0)),
                "governor_unseen_keep_eco_guard_enabled": bool(getattr(args, "governor_unseen_keep_eco_guard_enabled", 0)),
                ...
                "governor_long_event_prekeep_split_guard_enabled": bool(getattr(args, "governor_long_event_prekeep_split_guard_enabled", 0)),
                "governor_calm_compact_restore_enabled": bool(getattr(args, "governor_calm_compact_restore_enabled", 0)),
                "training_guard_guidance_enabled": bool(getattr(args, "training_guard_guidance_enabled", 0)),
                "scene_bias_weight": float(getattr(args, "scene_bias_weight", 0.0)),
                "low_level_overrides": low_level_overrides,
                "reward_energy_weight": float(getattr(args, "reward_energy_weight", 0.0)),
                "reward_speed_excess_weight": float(getattr(args, "reward_speed_excess_weight", 0.0)),
                "reward_accel_weight": float(getattr(args, "reward_accel_weight", 0.0)),
                "reward_energy_reference_kj": float(getattr(args, "reward_energy_reference_kj", 0.20)),
                ...
                "reward_tight_gap_weight": float(getattr(args, "reward_tight_gap_weight", 0.20)),
                "reward_safe_gap_bonus": float(getattr(args, "reward_safe_gap_bonus", 0.08)),
                "risk_bucket_distribution": policy.buffer.bucket_distribution() if hasattr(policy.buffer, "bucket_distribution") else {},
                "train_every_k_steps": int(getattr(args, "train_every_k_steps", 1)),
                "train_updates_per_trigger": int(getattr(args, "train_updates_per_trigger", 1)),
